...
AI Usage for Windows currently uses the SQLite/Cloud Code fallback path for
Antigravity. Direct Windows language-server discovery is intentionally left for
a later provider hardening pass.
...
- **Windows path:** `%APPDATA%\Antigravity\User\globalStorage\state.vscdb`
- **macOS fallback path:** `~/Library/Application Support/Antigravity/User/globalStorage/state.vscdb`
