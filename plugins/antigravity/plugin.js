(function () {
  ...
  function stateDbCandidates(ctx) {
    if (ctx.app && ctx.app.platform === "windows") return STATE_DB_CANDIDATES
    return [
      "~/Library/Application Support/Antigravity/User/globalStorage/state.vscdb",
      "%APPDATA%\\Antigravity\\User\\globalStorage\\state.vscdb",
      "~/AppData/Roaming/Antigravity/User/globalStorage/state.vscdb",
  ...
  function queryStateDb(ctx, sql) {
    var candidates = stateDbCandidates(ctx)
    var lastError = null
    for (var i = 0; i < candidates.length; i++) {
      try {
        return ctx.host.sqlite.query(candidates[i], sql)
      } catch (e) {
        lastError = e
      }
    }
  ...
  function probe(ctx) {
    ...
    if (tokens.length === 0 && !(proto && proto.refreshToken)) throw "Start Antigravity and try again."
  ...
  globalThis.__ai_usage_plugin = { id: "antigravity", probe: probe }
})()
