...
describe("antigravity plugin", () => {
  ...
  it("queries Windows Antigravity state DB path first", async () => {
    const ctx = makeCtx()
    const queriedPaths = []
    ctx.host.ls.discover.mockReturnValue(null)
    ctx.host.sqlite.query.mockImplementation((db, sql) => {
    ...
    expect(queriedPaths[0]).toBe("%APPDATA%\\Antigravity\\User\\globalStorage\\state.vscdb")
  })
  ...
  it("refreshes expired proto token even when no other token is available", async () => {
    const ctx = makeCtx()
    const pastExpiry = Math.floor(Date.now() / 1000) - 3600
    setupSqliteMock(ctx, null, makeProtobufBase64(ctx, "ya29.expired-token", "1//refresh", pastExpiry))
    ctx.host.ls.discover.mockReturnValue(null)
    ...
    ctx.host.http.request.mockImplementation((opts) => {
      const url = String(opts.url)
      if (url.includes("oauth2.googleapis.com")) {
        return { status: 200, bodyText: JSON.stringify({ access_token: "ya29.refreshed", expires_in: 3600 }) }
      }
      if (url.includes("fetchAvailableModels")) {
        capturedTokens.push(opts.headers.Authorization)
        if (opts.headers.Authorization === "Bearer ya29.refreshed") {
    ...
    expect(result.lines.length).toBeGreaterThan(0)
    expect(capturedTokens).toEqual(["Bearer ya29.refreshed"])
  })
